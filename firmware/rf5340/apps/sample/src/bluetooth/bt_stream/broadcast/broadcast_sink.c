...
#if defined(CONFIG_AURACLE_WIRE)
static uint32_t wire_broadcast_id;
#endif
...
{
	int idx = stream_index_get(stream);

#if defined(CONFIG_AURACLE_WIRE)
	wire_send_bis("streaming", wire_broadcast_id, (uint8_t)idx);
#endif
	...
	LOG_INF("Stream index %d started", idx);
...
#if defined(CONFIG_AURACLE_WIRE)
	wire_send_bis("stopped", wire_broadcast_id, (uint8_t)stream_index_get(stream));
#endif
}
...
#if defined(CONFIG_AURACLE_WIRE)
	wire_broadcast_id = broadcast_id;
#endif
