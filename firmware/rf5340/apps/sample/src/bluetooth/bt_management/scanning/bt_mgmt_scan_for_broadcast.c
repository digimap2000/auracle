...
static void scan_recv_cb(const struct bt_le_scan_recv_info *info, struct net_buf_simple *ad)
{
	...
	/* Log everything we see on the air */
	char addr_str[BT_ADDR_LE_STR_LEN];

	(void)bt_addr_le_to_str(info->addr, addr_str, BT_ADDR_LE_STR_LEN);
	if (source.id != INVALID_BROADCAST_ID) {
		LOG_INF("Broadcast found: name=\"%s\" id=0x%06x addr=%s rssi=%d",
			source.name[0] ? source.name : "(unnamed)",
			source.id, addr_str, info->rssi);
	} else {
		LOG_INF("Periodic advertiser (no broadcast UUID): addr=%s rssi=%d",
			addr_str, info->rssi);
	}
...
#if defined(CONFIG_AURACLE_WIRE)
	wire_send_sync("synced", brcast_src_info.id, &sync->addr);
#endif
...
#if defined(CONFIG_AURACLE_WIRE)
	wire_send_sync("lost", brcast_src_info.id, &sync->addr);
#endif
